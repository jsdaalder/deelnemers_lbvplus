...
This repo currently defines two OpenAI-powered “agents” in `4_ai_classify_lbv_and_addresses.py`. Both consume the combined HTML/PDF text from each publication and return structured JSON that feeds the downstream CSVs.

## 1. LBV classification agent

- **Location:** `SYSTEM_PROMPT_FULL` / `USER_TEMPLATE_FULL`.
...
- **Location:** `SYSTEM_PROMPT_ADDR_ONLY` / `USER_TEMPLATE_ADDR_ONLY`.
